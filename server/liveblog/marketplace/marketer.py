...
def blogs():
    ...
    try:
        response = _send_marketplace_api_request(
            MARKETPLACE_APP_URL, "blogs", request.args
        )
...
def marketers():
    ...
    try:
        response = _send_marketplace_api_request(
            MARKETPLACE_APP_URL, "marketers", request.args
        )
    except APIConnectionError as e:
        return api_response(str(e), 500)
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            logger.warning(
                "Exception on attempt to parse response from marketplace app: {}".format(
                    str(e)
                )
            )
...
@marketers_blueprint.route(
    "/api/marketplace/marketers/<marketer_id>/blogs", methods=["GET"]
)
...
def marketer_languages():
    try:
        response = _send_marketplace_api_request(
            MARKETPLACE_APP_URL, "languages", request.args
        )
    except APIConnectionError as e:
        return api_response(str(e), 500)
    ...
    if response.status_code == 200:
        ...
        for language_code in content:
            if language_code not in SUPPORTED_LANGUAGES["languages"].keys():
                logger.warning(
                    "Language code {} not in SUPPORTED_LANGUAGES".format(language_code)
                )
                continue
            response.append(
                {
                    "code": language_code,
                    "name": SUPPORTED_LANGUAGES["languages"][language_code],
                }
            )
        return api_response(response, 200)
    else:
        return api_error(
            "Unable to get languages from marketplace.", response.status_code
        )
