...
from liveblog.syndication.utils import (
    send_api_request,
    trailing_slash,
    validate_secure_url,
)
        ...
        if httpsurl:
            ...
            if check_auth and key_field:
                ...
                if not api_key:
                    return self._error(
                        field,
                        "Unable to find '{}' for the given resource url.".format(
                            key_field
                        ),
                    )
            ...
            if chech_auth_enabled:
                api_url = (
                    value
                    if webhook
                    else urllib.parse.urljoin(value, "syndication/blogs")
                )
                try:
                    response = send_api_request(
                        api_url, api_key, json_loads=False, timeout=5
                    )
                ...
                if response.status_code != 200:
                    return self._error(
                        field,
                        "Unexpected response code {} to url.".format(
