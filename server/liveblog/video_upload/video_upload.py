...
@video_upload_blueprint.route(
    "/api/video_upload/oauth2callback", methods=["GET", "POST"]
)
def oauth2callback():
    ...
    if access_error or yt_data is None:
        logger.warning(
            'Access denied in google auth or missing credentials. Error: "{}"'.format(
                access_error
            )
        )
