...
class ThemeSettingsTestCase(TestCase):
    ...
    def test_a_angular_save_theme_settings(self):
        ...
        angular_result = self.themeservice._save_theme_settings(
            self.angular_theme, angular_previous_theme
        )
        # Keep user settings, saved by user in database
        self.assertEqual(
            angular_previous_theme.get("settings").get("datetimeFormat"),
            angular_result[0].get("datetimeFormat"),
        )
        # Override the default value present in theme
        self.assertNotEqual(angular_result[1], angular_previous_theme.get("settings"))
        self.assertNotEqual(
            angular_result[1].get("postsPerPage"),
            angular_previous_theme.get("settings").get("postsPerPage"),
        )
        # Injected new value in theme
        self.assertNotEqual(
            len(angular_result[1]), len(angular_previous_theme.get("settings"))
        )
    ...
    def test_c_classic_save_theme_settings(self):
        ...
        classic_previous_theme = {
            ...
            "options": [
                ...
                },
                {
                    ...
                    "type": "select",
                    "options": [
                        {"value": False, "label": "Show below menu bar"},
                        {"value": True, "label": "Show above menu bar"},
                    ],
                    "default": False,
                },
                {
        ...
        classic_result = self.themeservice._save_theme_settings(
            self.classic_theme, classic_previous_theme
        )
        # Keep user settings, saved by user in database
        self.assertEqual(
            classic_previous_theme.get("settings").get("datetimeFormat"),
            classic_result[0].get("datetimeFormat"),
        )
        # Override the default value present in theme
        self.assertNotEqual(classic_result[1], classic_previous_theme.get("settings"))
        self.assertNotEqual(
            classic_result[1].get("postsPerPage"),
            classic_previous_theme.get("settings").get("postsPerPage"),
        )
        # Injected new value in theme
        self.assertNotEqual(
            len(classic_result[1]), len(classic_previous_theme.get("settings"))
        )
    ...
    def test_b_default_save_theme_settings(self):
        ...
        default_result = self.themeservice._save_theme_settings(
            self.default_theme, default_previous_theme
        )
        # Keep user settings, saved by user in database
        self.assertEqual(
            default_previous_theme.get("settings").get("datetimeFormat"),
            default_result[0].get("datetimeFormat"),
        )
        # Override the default value present in theme
        self.assertNotEqual(default_result[1], default_previous_theme.get("settings"))
        self.assertNotEqual(
            default_result[1].get("postsPerPage"),
            default_previous_theme.get("settings").get("postsPerPage"),
        )
        # Injected new value in theme
        self.assertNotEqual(
            len(default_result[1]), len(default_previous_theme.get("settings"))
        )
    ...
    def test_d_amp_save_theme_settings(self):
        ...
        amp_previous_theme = {
            ...
            "options": [
                ...
                {"name": "canComment", "type": None},
                {"name": "autoApplyUpdates", "type": None},
                {"name": "hasHighlights", "type": None},
                {"name": "permalinkDelimiter", "type": None},
                {"name": "stickyPosition", "type": None},
            ],
            "i18n": {
                "cs": {
                    "Highlights": "Hlavní body",
        ...
        amp_result = self.themeservice._save_theme_settings(
            self.amp_theme, amp_previous_theme
        )
        # Keep user settings, saved by user in database
        self.assertEqual(
            amp_previous_theme.get("settings").get("datetimeFormat"),
            amp_result[0].get("datetimeFormat"),
        )
        # Override the default value present in theme
        self.assertNotEqual(amp_result[1], amp_previous_theme.get("settings"))
        self.assertNotEqual(
            amp_result[1].get("postsPerPage"),
            amp_previous_theme.get("settings").get("postsPerPage"),
        )
        # Injected new value in theme
