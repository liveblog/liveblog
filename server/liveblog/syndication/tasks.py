...
from settings import (
    ...
    SYNDICATION_CELERY_COUNTDOWN,
    SYNDICATION_LIMIT_POSTS_SENT_TO_CONSUMER,
)
...
def send_post_to_consumer(self, syndication_out, producer_post, action="created"):
    ...
    try:
        consumers.send_post(syndication_out, {"items": items, "post": post}, action)
    except APIConnectionError as e:
        raise self.retry(
            exc=e,
            max_retries=SYNDICATION_CELERY_MAX_RETRIES,
            countdown=SYNDICATION_CELERY_COUNTDOWN,
        )


@celery.task(bind=True)
def send_posts_to_consumer(
    self, syndication_out, action="created", limit=LIMIT_POSTS, post_status="submitted"
):
    """
    ...
    for i in range(num_chunks):
        ...
        for producer_post in posts:
            ...
            post_data = {"items": items, "post": post}
...
def send_single_post_subtask(self, chained_result, syndication_out, post_data, action):
    try:
        ...
        raise self.retry(
            exc=err, max_retries=task_retries, countdown=SYNDICATION_CELERY_COUNTDOWN
        )
    ...
    if "webhook_url" in consumer:
        try:
            response = send_api_request(
                consumer["webhook_url"], None, method="GET", json_loads=False
            )
        except (Exception, APIConnectionError):
            logger.warning(
                'Unable to connect to webhook_url "{}"'.format(consumer["webhook_url"])
            )
            webhook_enabled = False
        else:
            if response.status_code == 200:
                logger.info(
                    'Connected to webhook_url "{}".'.format(consumer["webhook_url"])
                )
                webhook_enabled = True
            else:
                logger.warning(
                    'Unable to connect to webhook_url "{}", status: {}'.format(
                        consumer["webhook_url"], response.status_code
                    )
                )
                webhook_enabled = False
        ...
        push_notification(
            ...
            consumer={"_id": consumer["_id"], "webhook_enabled": webhook_enabled},
            updated=True,
        )
...
def check_api_status(self, producer_or_id):
    ...
    if "api_url" not in producer:
        ...
        try:
            ...
            response = send_api_request(
                api_url, producer["consumer_api_key"], json_loads=False
            )
        except (Exception, APIConnectionError):
            api_status = "invalid_url"
        else:
    ...
    push_notification(
        ...
        producer={"_id": producer["_id"], "api_status": api_status},
        updated=True,
    )
...
def unlink_syndicated_posts(producer_blog_id):
    ...
    for post in syndicated_posts:
        ...
        try:
            ...
            logger.error(
                'Unable to remove syndication relation for post "{}". Exc: "{}"',
                post_id,
                err,
            )
