...
class ProducerService(BaseService):
    ...
    def _send_api_request(
        ...
        if not api_url:
            raise ProducerAPIError(
                'Unable to get producer "{}" api url.'.format(producer_id)
            )

        try:
            response = send_api_request(
                api_url,
                producer["consumer_api_key"],
                method=method,
                args=request.args,
                data=data,
    ...
    def get_blogs(self, producer_id, json_loads=True):
        return self._send_api_request(
            producer_id, "syndication/blogs", json_loads=json_loads
        )
    ...
    def syndicate(
        ...
        if not update:
            ...
            method = "PATCH"
        return self._send_api_request(
            producer_id, url_path, method=method, data=data, json_loads=json_loads
        )
    ...
    def unsyndicate(self, producer_id, blog_id, consumer_blog_id, json_loads=True):
        ...
        return self._send_api_request(
            producer_id, url_path, method="DELETE", data=data, json_loads=json_loads
        )
...
def producer_blogs(producer_id):
    ...
    try:
        ...
        if response.status_code == 200:
            return api_response(
                response.content, response.status_code, json_dumps=False
            )
        else:
            return api_error(
                "Unable to get producer blogs.", _response_status(response.status_code)
            )


@producers_blueprint.route(
    "/api/producers/<producer_id>/blogs/<blog_id>", methods=["GET"]
)
def producer_blog(producer_id, blog_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return api_error(
                'Unable to get producer blog "{}".'.format(blog_id),
                _response_status(response.status_code),
            )


@producers_blueprint.route(
    "/api/producers/<producer_id>/blogs/<blog_id>/posts", methods=["GET"]
)
def producer_blog_posts(producer_id, blog_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return api_error(
                "Unable to get producer blog posts.",
                _response_status(response.status_code),
            )


def _create_producer_blogs_syndicate(
    producer_id, blog_id, consumer_blog_id, auto_publish, auto_retrieve, start_date=None
):
    producers = get_resource_service("producers")
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            return api_error(
                'Syndication already sent for blog "{}"'.format(blog_id), 409
            )
        else:
            return api_error(
                "Unable to syndicate producer blog.",
                _response_status(response.status_code),
            )


def _update_producer_blogs_syndicate(
    producer_id, blog_id, consumer_blog_id, auto_retrieve, start_date
):
    producers = get_resource_service("producers")
    in_service = get_resource_service("syndication_in")
    in_syndication = in_service.get_syndication(producer_id, blog_id, consumer_blog_id)
    ...
    try:
        ...
        if response.status_code == 200:
            syndication_in = in_service.get_syndication(
                producer_id, blog_id, consumer_blog_id
            )
            in_service.update(
                syndication_in["_id"],
                {"auto_retrieve": auto_retrieve, "start_date": start_date},
                syndication_in,
            )
            ...
            return api_error(
                "Unable to update blog syndication.",
                _response_status(response.status_code),
            )
    ...
    if not syndication_in:
        ...
        try:
            response = producers.unsyndicate(
                producer_id, blog_id, consumer_blog_id, json_loads=False
            )
        except ProducerAPIError as e:
            return api_response(str(e), 500)
        else:
            if response.status_code == 204:
                ...
                return api_error(
                    "Unable to unsyndicate producer blog.",
                    _response_status(response.status_code),
                )


@producers_blueprint.route(
    "/api/producers/<producer_id>/syndicate/<blog_id>",
...
def producer_blogs_syndicate(producer_id, blog_id):
    ...
    if request.method == "DELETE":
        ...
        return _update_producer_blogs_syndicate(
            producer_id, blog_id, consumer_blog_id, auto_retrieve, start_date
        )
    else:
        return _create_producer_blogs_syndicate(
            producer_id,
            blog_id,
            consumer_blog_id,
            auto_publish,
...
@producers_blueprint.route(
    "/api/producers/<producer_id>/check_connection", methods=["GET"]
)
