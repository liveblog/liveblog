...
def _generate_text_item(
    body_text="Test post body", published_date=datetime(2022, 1, 1)
):
    return {
        "item_type": "text",
        "text": body_text,
...
class TestGetModifiedDate:
    def test_get_modified_date_with_last_updated_post(self):
        mock_blog = {
            "last_updated_post": {"_updated": "2023-06-27"},
    ...
    def test_get_modified_date_without_last_updated_post(self):
        mock_blog = {"start_date": "2023-06-01"}
        result = get_modified_date(mock_blog)
...
class TestGetBaseImage:
    def test_get_base_image_with_base_image(self):
        item = {
            "meta": {
                "media": {"renditions": {"baseImage": "https://example.com/image.jpg"}}
            }
        }
