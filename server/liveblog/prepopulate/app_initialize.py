    ...
    [
        ...
        (
            ...
            (
                ...
                [
                    [("_updated", pymongo.ASCENDING)],
                    [("_id", pymongo.ASCENDING), ("_updated", pymongo.ASCENDING)],
                ],
                False,
            ),
        ),
...
class AppInitializeWithDataCommand(superdesk.Command):
    ...
    def run(
        ...
        for name, (file_name, index_params, do_patch) in __entities__.items():
            try:
                ...
                logger.exception(ex)
                logger.info(
                    "Exception loading entity {} from {}".format(name, file_name)
                )
    ...
    def import_file(
        self, entity_name, path, file_name, index_params, do_patch=False, force=False
    ):
        ...
        if index_params:
            for index in index_params:
                ...
                options = (
                    crt_index.pop()
                    if isinstance(crt_index[-1], dict) and isinstance(index, list)
                    else {}
                )

                try:
                    collection = app.data.mongo.pymongo(resource=entity_name).db[
                        entity_name
                    ]
                    ...
                    logger.info(
                        " - Not able to create index: %s. Collection %s not found",
                        index,
                        entity_name,
                    )
