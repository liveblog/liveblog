Feature: Themes operations
    ...
    Scenario: Upload a theme with satisfied dependencies
        ...
        When we post to "themes"
        """
        [{"name": "classic", "extends": "angular", "options": [{"name": "postsPerPage", "default": "26"}, {"name": "postOrder", "default": "newest_first"}]}]
        """
        When we get "/blogs"
        Then we get list with 1 items
        """
        {"_items": [{"title": "foo blog", "theme_settings": {"postsPerPage": "25", "postOrder": "newest_first"}}]}
        """
